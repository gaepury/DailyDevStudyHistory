...
- 서버리스 사용사례 개인 공부 
   ...
   - 다양한 플랫폼 콜백 연결
	   - 알람의 로직을 확장하기 위해 사용
         - 알람 봇 콜백을 사용하여 사용자가 직접, 모니터링 플랫폼의 기능을 확장하는 효과
   - 콜백 주소가 1개의 콜백 주소만 등록이 가능할때, 이를 받은후, 액션 내에서 여러개의 내부 시스템으로 전파해주는 역할로 사용하고 있습니다.(프록시 역할)
   - github 프로젝트에 버전 라벨 달아주기
      - 액션에서 github 파일을 읽어 프로젝트 버전 획득
      - https://img.shields.io 에 요청을 날려 SVG/PNG 이미지 생성
